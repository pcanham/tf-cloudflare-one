...
resource "cloudflare_zero_trust_gateway_policy" "http_bypass_app_devtooling01" {
  account_id  = var.cloudflare_account_id
  action      = "off"
  description = "BUG REPORT 01 - Do not inspect"
  enabled     = true
  filters     = ["http"]
  name        = "BUG REPORT 01 - Development Tooling"
  precedence  = 999992
  traffic     = "any(app.hosts_ids[*] in {536 687 1238})"
  rule_settings {
    block_page_enabled                 = false
    insecure_disable_dnssec_validation = false
    ip_categories                      = false
